#Elizabeth Bach
#10 February 2015
#Using Mixed Model Regressions to parse correlations between fixed & random factors
#see Nakagawa &  Schielzeth 2013, Methods in Ecology and Evolution

#Code for calculating marginal and conditionl r2 from Lefcheck 2014, https://github.com/jslefche/rsquared.glmm
#Run this code for function "rsquared.glmm()"
...
#Read in aggregate mean weighted diameter (MWD) data
#use "Bach_COBS2012_MWD.csv" in COBS2012_sampling
MWD.2012<-read.csv(file.choose())
str(MWD.2012)

#Read in Enzyme data
#use "COBS2012_EnzySummaryGramMasterNoOutlier.csv" in COBS2012_Sampling/AggStats/AggEnzy
COBS2012.data<-read.csv(file.choose())

BX2012.data<-cbind(COBS2012.data[,1:7], COBS2012.data[10])

#Regress LM EEA against MWD using mixed model
library(lme4)
library(lmerTest)
library(bbmle)
library(plyr)
library(nlme)
#Run function in "Function_R2MixedModels.R" in COBS2012_Sampling\COBS_AggStats
#This contains the "rsquared.glmm()" function used below

#BX regression model, annotated code
#Subset to isolate LM
BX2012.LM<-droplevels(subset(BX2012.data, BX2012.data$Aggregate.Fraction=="1"))
BX.2012<-data.frame(cbind(BX2012.LM[,1:8], MWD.2012[,1:5]))
str(BX.2012)
head(BX.2012)

BX2012.lme<-lmer(BX~MWD+(1|Crop)+(1|Date)+(1|Block), data=BX.2012, REML=FALSE)
BX2012.lme2<-lmer(BX~MWD+Crop+Date+(1|Block), data=BX.2012, REML=FALSE)
summary(BX2012.lme)
#Use random effects version, Crop Date explain same variance as random or fixed, 
#when random, enables me to parse out MWD effect more cleanly
summary(BX2012.lme2)
#Correlation of fixed effects is actually the correlation between the slopes of the fixed effect coefficients
#MWD and EEA are changing in the same direction and the slopes are 0.99 correlated
#cor.test(BX.2012$BX,BX.2012$MWD, method = "pearson")
#This test gives you Pearson's correlation, the correltaion between the actual variables
#Can also use Spearman's rho or Kendall's tao for non-normally distributed data
#cor.test(BX.2012$BX,BX.2012$MWD, method = "spearman")
#cor.test(BX.2012$BX,BX.2012$MWD, method = "kendall")
#In this case, Spearman and Pearson come out about the same, the data skewed, so Spearman may be better
#hist(BX.2012$BX)
rsquared.glmm(BX2012.lme)
#Marginal R2 is variance described by fixed effect, conditional includes fixed+random
